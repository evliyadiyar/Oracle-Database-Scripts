--------------------------------------------------------------------------------
--  WaitLockEvents.sql
--  Oracle RAC ve Standalone için "Bekleyen ↔ Bloklayan" Oturumları Gösterir
--  Yazar: Evliya Diyar | https://github.com/evliyadiyar/
--  Açıklama:
--      Bu sorgu, Oracle veritabanında bekleyen ve bloklayan oturumları
--      "tek shot" olarak gösterir. RAC ortamlarında da çalışır.
--      Hangi oturumun, hangi nesnede, hangi SQL ile ve ne kadar süredir
--      beklediğini ve blokladığını özetler.
--
--  Kullanım:
--      SQL*Plus veya başka bir SQL aracı ile çalıştırabilirsiniz.
--      Yöneticiler ve DBA'lar için pratik analiz imkanı sunar.
--------------------------------------------------------------------------------
...
WITH links AS (
  ...
  FROM
    gv$session w
  WHERE
    w.wait_class <> 'Idle' -- Sadece aktif bekleyenler
    AND (w.blocking_session IS NOT NULL OR w.final_blocking_session IS NOT NULL)
)
SELECT
  l.w_inst, l.w_sid, l.w_serial, l.w_user, l.w_sql_id,
...
--------------------------------------------------------------------------------
-- Açıklamalar:
--  b_sql_id:   Bloklayanın SQL'i. Oturum "idle in transaction" ise SQL_ID boş olabilir.
--              Bu nedenle COALESCE(b.sql_id, b.prev_sql_id) kullanılmıştır.
--  tx_start:   Transaction'ın ne zamandır açık olduğunu gösterir.
--              (Uzun süre açık transaction tipik bloklayıcıdır.)
--  row_object: Eğer satır bekleniyorsa (TX), hangi obje üzerinde beklediğini gösterir.
--              (V$SESSION’deki ROW_WAIT_* sütunlarıyla bağlantılıdır.)
--
--
